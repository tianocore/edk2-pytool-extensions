...
class CodeQlTests(unittest.TestCase):
    ...
    def test_codeql_option(self) -> None:
    ...
    def test_codeql_enabled_linux(self, mock_host_info: MagicMock) -> None:
    ...
    @patch("edk2toolext.codeql.GetHostInfo")
    def test_codeql_enabled_windows(self, mock_host_info: MagicMock) -> None:
    ...
    def test_codeql_not_enabled(self) -> None:
        """Tests that CodeQL is recognized on the command-line properly."""
        mock_args = Namespace(codeql=False)
        result = codeql.is_codeql_enabled_on_command_line(mock_args)
        self.assertFalse(result)

    def test_set_audit_only_mode(self) -> None:
