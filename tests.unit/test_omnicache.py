...
class TestOmniCache(unittest.TestCase):
    ...
    def test_omnicache_new(self):
        ...
        gitret = utility_functions.RunCmd(
            "git", "config --local omnicache.metadata.version", workingdir=testcache, outstream=out
        )
        assert gitret == 0
        ...
        gitret = utility_functions.RunCmd(
            "git", "config --local --unset omnicache.metadata.version", workingdir=testcache
        )
        assert gitret == 0
    ...
    def test_omnicache_convert(self):
        ...
        for name, url in remotes.items():
            if url == "https://github.com/tianocore/edk2-pytool-extensions.git":
                out = StringIO()
                gitret = utility_functions.RunCmd(
                    "git", f"config --local omnicache.{name}.displayname", workingdir=testcache, outstream=out
                )
                assert gitret == 0
