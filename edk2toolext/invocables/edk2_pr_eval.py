    ...
    def get_packages_to_build(self, possible_packages: list) -> dict:
        ...
        if PlatformDscInfo is not None and len(remaining_packages) > 0:
            ...
            for target in self.requested_target_list:
                ...
                # get the build variables resolved against the current target
                input_vars = shell_environment.GetBuildVars().GetAllBuildKeyValues(target)

                # update any values explicitly passed by the GetPlatformDscAndConfig() implementation
