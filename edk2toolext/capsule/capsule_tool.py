...
def get_cli_options(args: Any = None) -> argparse.Namespace:  # noqa: ANN401
    ...
    parser.add_argument(
        "capsule_payload", type=argparse.FileType("rb"), help="a filesystem path to the binary payload for the capsule"
    )
    parser.add_argument(
        "output_dir",
...
def main() -> None:
    ...
    missing_capsule_options = tuple(
        option for option in required_capsule_options if option not in final_options["capsule"]
    )
    ...
    uefi_capsule_header = capsule_helper.build_capsule(
        args.capsule_payload.read(), final_options["capsule"], signer, final_options["signer"]
    )
